...
**Done & shipped:** all 6 roadmap items + the V2 batch. The dreaming layer
ingests atone, affirm, memory, sessions, pinned, **claude-audit** (now 12+
registered domains), runs grounded per-domain + cross-domain dream passes,
surfaces via daily digest / SessionStart / widget / **`i-dream board`**, exposes
a formal **ingestion contract** (`i-dream contract`), and self-sustains via
**scheduled crons** (dream-pass 02:45 → digest 03:00 → weekly audit Sun 02:30).

**V2 shipped 2026-05-23:** scheduled dream-pass cron · weekly-audit cron
(`audit run --non-interactive` stages proposals) · `i-dream thread
{add,list,resolve,reopen}` (B Stage 7) · `i-dream board` static 2×2 dashboard
(B Stage 4 half). **claude-audit went live** — its agent built the hook-feedback
system + activated it; validated: grounded insights + the first live
cross-domain severity-weighted join (hook-friction ↔ atone mistake slugs).

**What's left — all optional, none on the critical path:**

| Item | Owner | Notes |
|------|-------|-------|
| Widget graph-view dedup | i-dream | The one deferred piece of the widget rebuild — collapse PatternGraphView + AssociationGraphView (~1100 lines) into a shared base. Untestable without live dashboard interaction; deferred from the 2026-05-31 rebuild. [`docs/22`](./22-widget-audit.md). |
| Literal multi-Agent audit dispatch | i-dream | V2 — audit runs today as one richer prompt structured as 7 voices; literal fan-out is the refinement. |
...
| Fully-interactive `board` TUI | i-dream | Optional — current board is a static snapshot; ratatui scroll/nav is the upgrade (deferred: needs interactive testing). |
| claude-audit `heeded` enrichment | claude-audit | Its extractor isn't populating the telemetry join yet (`heeded: null`), so the fire-but-ignored signal is dormant. |
...
| 2026-05-31 | **Widget rebuild SHIPPED** (Tier 0+1+2, merged `fdc527d`). The owner's biggest pain, on a new Mac. Non-blocking `DataStore` (menu-open does zero sync disk reads), menu re-topped with reflect/review **outcome** (not vanity activity), ~560 lines dead code removed + 6 menu items cut + frequency 12→4 presets + insight rows wrapped + fonts unified + plan-ref comments stripped. Added `i-dream reflect --json`. Skeptical-review caught 5 missed sync reads + a trend-drift risk + a pipe-deadlock — all fixed. Graph-view dedup deferred (untestable refactor). Process note: a parallel session sharing the git checkout commingled branches; resolved by merging the combined line (incl. 2 audit/dream-pass fixes) to master. |
| 2026-05-25 | **Widget audit** — 3 parallel grounded passes (UX, fault-tolerance, architecture+perf) on the 9,554-line `i-dream-bar.swift`. Convergent: it shows *activity* not *outcome* (reflect/review unwired, 0 calls); clunk + freeze share one root cause (a blocking subprocess on the main thread, `:5800`); accretion is structural (450-line hard-coded menu + duplicated load); blast radius is contained (safe). Full plan + pickup context in [`docs/22`](./22-widget-audit.md): tiered, felt-value-first, **rewire+subtract not rewrite**. Recommended: Tier 0+1+2 (wire reflect/review + non-blocking DataStore + cut ~1500 lines), defer decomposition. **Not started.** Also wrote `~/.claude/i-dream/MIGRATION.md` (bring the dreaming data to a new Mac). |
| 2026-05-24/25 | **Felt-value pivot** (user: "claude keeps adding features without a felt improvement; I have no idea how the dreamt stuff helps"). Reframed around closing the dream→behavior loop, value-first not feature-first. Shipped: (1) sharpened the SessionStart injection — actionable prechecks instead of bare slugs, filtered out i-dream's self-referential rules, 🔴 blind-spot escalation for patterns recurring despite warnings (in `~/.claude/scripts/dream/dream-insights.sh`); (2) `i-dream reflect` — audits whether guidance lands (recurrence trend per pattern); (3) `i-dream review` + Monday-09:00 cron + Calendar event — the weekly review comes TO the user (push, since they never log out). `#1`+`#2` of the felt-value goals now whole. Hook-graduation ("better option") explored + designed in [`docs/21`](./21-hook-graduation.md) but **gated, not built** — awaits real-data validation that the soft escalation isn't enough. |
| 2026-05-23 | **V2 batch** (4 features, all reviewed + fixed): scheduled dream-pass cron + weekly-audit cron (cron.rs generalized to a JOBS registry; `audit run --non-interactive` stages proposals); `i-dream thread {add,list,resolve,reopen}` (B Stage 7, digest "Open threads" now live); `i-dream board` static 2×2 dashboard (B Stage 4 half). **claude-audit went live** — its agent built + activated the hook-feedback system since the 05-22 staging; the board surfaced it, validation passed (grounded insights + first live cross-domain severity-weighted join: hook slugs ↔ atone mistake slugs). 350 tests. |
